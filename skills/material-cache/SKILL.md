...
> **Общий кеш.** `knowledge/` — это общая база предметных материалов (теория/разборы), не привязанная к пользователю. В ней НЕТ приватных данных пользователей (профилей, планов, прогресса). Это намеренно: материал по «закону Кулона» одинаков для всех, кешировать его по разу выгоднее по токенам. Персональная привязка «кто что разобрал» живёт в `users/<user_key>/materials/` (скилл `goal-materials`).
...
### Согласованность
- `knowledge/` можно синхронизировать с Obsidian vault владельца, если это настроено в `AGENTS.md`/`TOOLS.md`. По умолчанию — только локально.
...
- `goal-materials` — персональная привязка материалов к пользователю (этот скилл — общий кеш)
